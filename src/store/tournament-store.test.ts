...
describe('useTournamentStore', () => {
  ...
  it('starts with empty tournament progress', () => {
    ...
    assert.equal(state.startTimestamp, null);
  ...
  it('starts from today by consuming official results through the date', () => {
    ...
    assert.ok(state.startTimestamp);
    assert.equal(
      new Date(state.startTimestamp).getTime(),
  ...
  it('anchors from-today knockout timelines on the selected calendar date', () => {
    ...
    assert.ok(anchoredClock.getTime() >= new Date(state.startTimestamp).getTime());
  });
